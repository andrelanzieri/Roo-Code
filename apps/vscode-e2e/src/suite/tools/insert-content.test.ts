...
${testFile.content}`
		...
		const messageHandler = ({ message }: { message: ClineMessage }) => {
			...
			if (message.type === "say" && message.say === "api_req_started" && message.text) {
				...
				if (
					text.includes("insert_content") ||
					text.includes('"tool":"insert_content"') ||
					text.includes("insertContent")
				) {
					...
					try {
						...
						if (requestData && requestData.request && requestData.request.includes("insert_content")) {
							insertContentExecuted = true
							console.log("insert_content tool executed (parsed from JSON)!")
						}
					} catch (e) {
