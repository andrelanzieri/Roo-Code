    ...
    suspend fun vscode_changes(title: String, changes: List<List<Map<String, Any?>>>): Any? {
        ...
        try {
            ...
            for ((index, changeTriplet) in changes.withIndex()) {
                ...
                val originalFileUri = ai.roocode.jetbrains.editor.createURI(changeTriplet[0])
                val beforeContentUri = ai.roocode.jetbrains.editor.createURI(changeTriplet[1])
                val afterContentUri = ai.roocode.jetbrains.editor.createURI(changeTriplet[2])
