...
package ai.roocode.jetbrains.plugin
...
import ai.roocode.jetbrains.core.ExtensionProcessManager
import ai.roocode.jetbrains.core.ExtensionSocketServer
...
import ai.roocode.jetbrains.core.*
import ai.roocode.jetbrains.util.ExtensionUtils
...
class WecoderPlugin : StartupActivity.DumbAware {
    ...
    override fun runActivity(project: Project) {
        ...
        LOG.info(
            "Initializing Roo Code plugin for project: ${project.name}, " +
        ...
        try {
            ...
            Disposer.register(project, Disposable {
                LOG.info("Disposing Roo Code plugin for project: ${project.name}")
                pluginService.dispose()
                SystemObjectProvider.dispose()
            })

            LOG.info("Roo Code plugin initialized successfully for project: ${project.name}")
        } catch (e: Exception) {
            LOG.error("Failed to initialize Roo Code plugin", e)
        }
    }
}
...
class WecoderPluginService(private var currentProject: Project) : Disposable {
    ...
    companion object {
        // Debug mode switch
        @Volatile
        private var DEBUG_TYPE: DEBUG_MODE = ai.roocode.jetbrains.plugin.DEBUG_MODE.NONE
        ...
        init {
            try {
                ...
                val configStream: InputStream? = WecoderPluginService::class.java.getResourceAsStream("/ai/roocode/jetbrains/plugin/config/plugin.properties")
...
//                ServiceProxyRegistry.getInstance().initialize()
                ...
                if (DEBUG_TYPE == ai.roocode.jetbrains.plugin.DEBUG_MODE.ALL) {
                    ...
                    // In debug modes, prefer TCP socket for compatibility with Node host bridge
                    val server: ISocketServer =
                        if (WecoderPluginService.getDebugMode() != DEBUG_MODE.NONE) {
                            socketServer
                        } else {
                            if (SystemInfo.isWindows) socketServer else udsSocketServer
                        }
    ...
    private fun cleanup() {
        try {
            ...
            if (DEBUG_TYPE == ai.roocode.jetbrains.plugin.DEBUG_MODE.NONE) {
                processManager.stop()
            }
        } catch (e: Exception) {
