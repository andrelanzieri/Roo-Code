...
import ai.roocode.jetbrains.plugin.DEBUG_MODE
import ai.roocode.jetbrains.plugin.WecoderPluginService
import ai.roocode.jetbrains.util.PluginResourceUtil
import ai.roocode.jetbrains.util.ProxyConfigUtil
import java.io.File
import java.util.concurrent.TimeUnit
import ai.roocode.jetbrains.util.ExtensionUtils
import ai.roocode.jetbrains.util.PluginConstants
import ai.roocode.jetbrains.util.NotificationUtil
import ai.roocode.jetbrains.util.NodeVersionUtil
import ai.roocode.jetbrains.util.NodeVersion
import ai.roocode.jetbrains.i18n.I18n
...
//                "--trace-uncaught",
    ...
    fun findExtensionEntryFile(): String? {
        ...
        // Normal mode: prefer runtime/src/main.js, fallback to runtime/main.js
        val primaryPath = ai.roocode.jetbrains.util.PluginResourceUtil.getResourcePath(PLUGIN_ID, "$RUNTIME_DIR/src/$EXTENSION_ENTRY_FILE")
        if (primaryPath != null) {
            val file = java.io.File(primaryPath)
            if (file.exists() && file.isFile) {
                return primaryPath
            }
        }
        val fallbackPath = ai.roocode.jetbrains.util.PluginResourceUtil.getResourcePath(PLUGIN_ID, "$RUNTIME_DIR/$EXTENSION_ENTRY_FILE")
        if (fallbackPath != null) {
            val file = java.io.File(fallbackPath)
            if (file.exists() && file.isFile) {
                return fallbackPath
            }
        }
